...
class Retriever(MultiVectorRetriever):
    ...
    def documents_loader(self, folder_path):
    ...
    def make_batch_chunks(self, chunks, max_batch_size: int):
        """ tạo batch cho việc thêm tài liệu vào retriever"""
        for i in range(0, len(chunks) , max_batch_size):
            if( i + max_batch_size < len(chunks)):
                yield chunks[i:i+max_batch_size]
            else: yield chunks[i:i+(len(chunks)-i)]

    # hàm thêm tài liệu vào retriever
    def add_documents_to_retriever(self, data_path: str, chunk_size: int = 300, chunk_overlap: int = 50, max_batch_size: int = 166):
    ...
    def multi_query(self, queries: list[str], top_k: int) :
    ...
    def re_ranking(self, results: list[ list ], top_k: int, k : int = 60):
        ...
        rerank_results = [
            (loads(doc_str) , score) for doc_str, score in sorted(scores.items(), key=lambda item: item[1], reverse=True)
        ]
...
# retriever = Retriever( api_key=API_KEY,embedding_model= EMBEDDING_MODEL)
# retriever.add_documents_to_retriever(DATA_PATH, CHUNK_SIZE, OVERLAP_SIZE, MAX_BATCH_SIZE)


# queries = ["giới thiệu về học viện" , "thông tin về học viện" , "thông tin về học viện công nghệ bưu chính viễn thông"]

# retrieved_results = retriever.multi_query(queries, 5)
